...
import {
  ChevronLeft, Calendar, Clock, Plus, Pencil, X, CheckCircle2, Check,
...
export default function AddPillScreen({ navigation }: Props) {
  ...
  const {
    ...
    handleSave,
  } = useAddPillForm(() => navigation.navigate('MainTabs'));
  ...
  const finalizeTime = (dateToUse?: Date) => {
    ...
    if (pickerMode === 'add') {
      ...
      editScheduleTime(selectedIndex, t);
    }
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: c.background }]}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          ...
          >
            <Text style={[styles.sectionLabel, { color: c.muted }]}>Medication Details</Text>
            <View style={[styles.card, { backgroundColor: c.card, borderColor: c.border }]}>
              <Text style={[styles.fieldLabel, { color: c.subtext }]}>Medicine Name</Text>
              <TextInput
                style={[styles.fieldInput, { color: c.text, borderBottomColor: nameFocused ? c.primary : c.border }]}
                ...
                returnKeyType="next"
              />
              <Text style={[styles.fieldLabel, { color: c.subtext, marginTop: 14 }]}>Total Pills to Load</Text>
              <TextInput
                style={[styles.fieldInput, { color: c.text, borderBottomColor: stockFocused ? c.primary : c.border }]}
                placeholder="e.g. 30"
                placeholderTextColor={c.muted}
            ...
            <Text style={[styles.sectionLabel, { color: c.muted }]}>Start Date</Text>
            <View style={[styles.card, { backgroundColor: c.card, borderColor: c.border }]}>
              <TouchableOpacity
              ...
              >
                <View style={{ flex: 1 }}>
                  <Text style={[styles.fieldLabel, { color: c.subtext }]}>Begin course on</Text>
                  <Text style={[styles.dateValue, { color: isFromToday ? c.muted : c.text }]}>
                    {isFromToday
                      ? `Today — ${formatDate(new Date())}`
                      : startDate ? formatDate(startDate) : 'Tap to choose a date'}
                  </Text>
                </View>
                {!isFromToday && <Calendar size={18} color={c.primary} strokeWidth={2} />}
              </TouchableOpacity>

              <View style={[styles.divider, { backgroundColor: c.divider }]} />

              <TouchableOpacity style={styles.checkRow} onPress={toggleStartFromToday} activeOpacity={0.7}>
                ...
                <Text style={[styles.checkLabel, { color: c.text }]}>Start from today</Text>
              </TouchableOpacity>
            </View>

            <View style={styles.scheduleHeader}>
              <Text style={[styles.sectionLabel, { color: c.muted }]}>Dosage Times</Text>
              <TouchableOpacity
            ...
            {schedule.length === 0 ? (
              <View style={[styles.emptyState, { borderColor: c.border }]}>
                ...
                <Text style={[styles.emptyTitle, { color: c.muted }]}>No dose times yet</Text>
                <Text style={[styles.emptyHint, { color: c.muted }]}>Tap "Add Time" to schedule doses</Text>
              </View>
            ) : (
              schedule.map((time, index) => (
                <View key={index} style={[styles.timeCard, { backgroundColor: c.card, borderColor: c.border }]}>
                  <View style={[styles.timeIconWrap, { backgroundColor: c.primary + '18' }]}>
                    <Clock size={16} color={c.primary} strokeWidth={2} />
                  </View>
                  <TouchableOpacity style={styles.timeInfo} onPress={() => openEditTime(index)} activeOpacity={0.7}>
                    <Text style={[styles.timeValue, { color: c.text }]}>{formatTime(time)}</Text>
                    <View style={[styles.editChip, { backgroundColor: c.inputBg }]}>
                      <Pencil size={10} color={c.muted} strokeWidth={2} />
                      <Text style={[styles.editChipText, { color: c.muted }]}>Edit</Text>
                    </View>
                  </TouchableOpacity>
                  <TouchableOpacity onPress={() => removeScheduleTime(index)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
        ...
        <View style={[styles.footer, { backgroundColor: c.background, borderTopColor: c.border }]}>
          <TouchableOpacity
            style={[styles.saveBtn, { backgroundColor: isFormValid ? c.primary : c.inputBg }]}
            onPress={handleSave}
...
const styles = StyleSheet.create({
  ...
  card: { borderRadius: 18, borderWidth: 1, padding: 18, marginBottom: 24, gap: 4 },
  fieldLabel: { fontSize: 12, fontWeight: '600', marginBottom: 4 },
  fieldInput: { fontSize: 18, fontWeight: '600', paddingVertical: 6, borderBottomWidth: 1.5, marginBottom: 4 },
  divider: { height: 1, marginVertical: 14 },
  dateRow: { flexDirection: 'row', alignItems: 'center' },
  dateValue: { fontSize: 16, fontWeight: '600', marginTop: 2 },
  ...
  addTimeBtn: { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 14, paddingVertical: 8, borderRadius: 10, marginBottom: 10 },
  addTimeBtnText: { color: '#fff', fontSize: 13, fontWeight: '700' },
  emptyState: { padding: 36, alignItems: 'center', gap: 8, borderWidth: 1.5, borderRadius: 18, marginBottom: 12 },
  emptyTitle: { fontSize: 15, fontWeight: '600' },
  emptyHint: { fontSize: 13, textAlign: 'center', opacity: 0.7 },
  timeCard: { flexDirection: 'row', alignItems: 'center', borderRadius: 14, borderWidth: 1, padding: 14, marginBottom: 10, gap: 12 },
  timeIconWrap: { width: 36, height: 36, borderRadius: 10, justifyContent: 'center', alignItems: 'center' },
  timeInfo: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 10 },
  timeValue: { fontSize: 17, fontWeight: '700' },
  editChip: { flexDirection: 'row', alignItems: 'center', gap: 3, paddingHorizontal: 7, paddingVertical: 3, borderRadius: 6 },
  editChipText: { fontSize: 11, fontWeight: '600' },
  footer: { paddingHorizontal: 20, paddingVertical: 16, borderTopWidth: 1 },
  saveBtn: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 10, height: 56, borderRadius: 14 },
  saveBtnText: { fontSize: 15, fontWeight: '700' },
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.45)', justifyContent: 'flex-end' },
  sheet: { borderTopLeftRadius: 26, borderTopRightRadius: 26, paddingHorizontal: 24, paddingTop: 14, paddingBottom: Platform.OS === 'ios' ? 36 : 24 },
  ...
  cancelBtn: { flex: 1, height: 50, borderRadius: 12, borderWidth: 1.5, justifyContent: 'center', alignItems: 'center' },
